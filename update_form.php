    ...
    switch ($step) {
        ...
        case 5:
            ...
            if (isset($_FILES['photo']) && $_FILES['photo']['error'] == UPLOAD_ERR_OK) {
                $photoPath = 'uploads/' . uniqid() . '-' . basename($_FILES['photo']['name']);
                move_uploaded_file($_FILES['photo']['tmp_name'], $photoPath);
            }

            if (isset($_FILES['signature']) && $_FILES['signature']['error'] == UPLOAD_ERR_OK) {
                $signaturePath = 'uploads/' . uniqid() . '-' . basename($_FILES['signature']['name']);
                move_uploaded_file($_FILES['signature']['tmp_name'], $signaturePath);
            }

            $sql = "UPDATE applicants SET photo_path=?, signature_path=?, is_submitted =? WHERE id=?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("sssi", $photoPath, $signaturePath, $isSubmitted, $user_id);
            break;
    }
